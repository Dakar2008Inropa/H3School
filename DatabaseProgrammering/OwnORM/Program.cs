    ...
    public static class Program
    {
        ...
        private static async Task ListStudentsAsync(SchoolRepository repo, CancellationToken cancellationToken)
        {
            try
            {
                ...
                table.AddColumns("[bold]Elevtype[/]");
        ...
        private static StudentType PromptStudentType()
        {
            SelectionPrompt<string> prompt = new SelectionPrompt<string>()
                .Title("[cyan]Vælg elevtype[/]:")
                .AddChoices("EUD", "HTX", "EUX");

            string choice = AnsiConsole.Prompt(prompt);

            return Enum.Parse<StudentType>(choice);
        }

        private static StudentType PromptOptionalStudentType(StudentType current)
        {
            TextPrompt<string> prompt = new TextPrompt<string>($"[cyan]Ny elevtype (EUD/HTX/EUX) (nuværende: {current})[/]:").AllowEmpty();
            ...
            if (Enum.TryParse<StudentType>(input, true, out StudentType parsed))
                return parsed;

            AnsiConsole.MarkupLine("[yellow]Ugyldig elevtype angivet. Beholder nuværende.[/]");
            return 0;
        }
